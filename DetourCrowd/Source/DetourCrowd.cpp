...
inline dtFloat tween(const dtFloat t, const dtFloat t0, const dtFloat t1)
{
	return dtClamp((t-t0) / (t1-t0), 0.0f, 1.0f);
}

static void integrate(dtCrowdAgent* ag, const dtFloat dt)
{
	// Fake dynamic constraint.
	const dtFloat maxDelta = ag->params.maxAcceleration * dt;
	dtFloat dv[3];
	dtVsub(dv, ag->nvel, ag->vel);
	dtFloat ds = dtVlen(dv);
	if (ds > maxDelta)
...
static bool overOffmeshConnection(const dtCrowdAgent* ag, const dtFloat radius)
{
	if (!ag->ncorners)
		return false;
	...
	{
		const dtFloat distSq = dtVdist2DSqr(ag->npos, &ag->cornerVerts[(ag->ncorners-1)*3]);
...
static dtFloat getDistanceToGoal(const dtCrowdAgent* ag, const dtFloat range)
{
	if (!ag->ncorners)
		return range;
...
static void calcSmoothSteerDirection(const dtCrowdAgent* ag, dtFloat* dir)
{
	if (!ag->ncorners)
	{
		dtVset(dir, 0,0,0);
	...
	const dtFloat* p0 = &ag->cornerVerts[ip0*3];
	const dtFloat* p1 = &ag->cornerVerts[ip1*3];
	
	dtFloat dir0[3], dir1[3];
	dtVsub(dir0, p0, ag->npos);
	dtVsub(dir1, p1, ag->npos);
	dir0[1] = 0;
	dir1[1] = 0;
	
	dtFloat len0 = dtVlen(dir0);
	dtFloat len1 = dtVlen(dir1);
	if (len1 > 0.001f)
...
static void calcStraightSteerDirection(const dtCrowdAgent* ag, dtFloat* dir)
{
	if (!ag->ncorners)
	{
		dtVset(dir, 0,0,0);
...
static int addNeighbour(const int idx, const dtFloat dist,
...
static int getNeighbours(const dtFloat* pos, const dtFloat height, const dtFloat range,
						 ...
						 dtCrowdAgent** agents, const int /*nagents*/, dtProximityGrid* grid)
{
	...
	for (int i = 0; i < nids; ++i)
	{
		...
		if (dtMathFabsf(diff[1]) >= (height+ag->params.height)/2.0f)
			continue;
		diff[1] = 0;
		const dtFloat distSqr = dtVlenSqr(diff);
...
bool dtCrowd::init(const int maxAgents, const dtFloat maxAgentRadius, dtNavMesh* nav)
{
	purge();
...
int dtCrowd::addAgent(const dtFloat* pos, const dtCrowdAgentParams* params)
{
	// Find empty slot.
	int idx = -1;
	for (int i = 0; i < m_maxAgents; ++i)
...
bool dtCrowd::requestMoveTargetReplan(const int idx, dtPolyRef ref, const dtFloat* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
bool dtCrowd::requestMoveTarget(const int idx, dtPolyRef ref, const dtFloat* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
...
bool dtCrowd::requestMoveVelocity(const int idx, const dtFloat* vel)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
void dtCrowd::updateMoveRequest(const dtFloat /*dt*/)
{
...
void dtCrowd::updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const dtFloat dt)
{
	if (!nagents)
		return;
	
	const dtFloat OPT_TIME_THR = 0.5f; // seconds
...
void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const dtFloat dt)
{
	static const int CHECK_LOOKAHEAD = 10;
	static const dtFloat TARGET_REPLAN_DELAY = 1.0; // seconds
...
void dtCrowd::update(const dtFloat dt, dtCrowdAgentDebugInfo* debug)
{
	m_velocitySampleCount = 0;
	...
	{
		dtCrowdAgent* ag = agents[i];
		const dtFloat* p = ag->npos;
		const dtFloat r = ag->params.radius;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		const dtFloat updateThr = ag->params.collisionQueryRange*0.25f;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		{
			const dtFloat* target = &ag->cornerVerts[dtMin(1,ag->ncorners-1)*3];
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		// Check 
		const dtFloat triggerRadius = ag->params.radius*2.25f;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		else
		{
			...
			const dtFloat slowDownRadius = ag->params.radius*2;	// TODO: make less hacky.
			const dtFloat speedScale = getDistanceToGoal(ag, slowDownRadius) / slowDownRadius;
		...
		{
			const dtFloat separationDist = ag->params.collisionQueryRange; 
			const dtFloat invSeparationDist = 1.0f / separationDist; 
			const dtFloat separationWeight = ag->params.separationWeight;
			
			dtFloat w = 0;
			dtFloat disp[3] = {0,0,0};
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				const dtFloat distSqr = dtVlenSqr(diff);
				if (distSqr < DT_FLT_EPSILON)
					continue;
				if (distSqr > dtSqr(separationDist))
					continue;
				const dtFloat dist = dtMathSqrtf(distSqr);
				const dtFloat weight = separationWeight * (1.0f - dtSqr(dist*invSeparationDist));
			...
			if (w > DT_FLT_EPSILON)
			{
				...
				const dtFloat speedSqr = dtVlenSqr(dvel);
				const dtFloat desiredSqr = dtSqr(ag->desiredSpeed);
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (ag->params.updateFlags & DT_CROWD_OBSTACLE_AVOIDANCE)
		{
			...
			{
				const dtFloat* s = ag->boundary.getSegment(j);
	...
	// Handle collisions.
	static const dtFloat COLLISION_RESOLVE_FACTOR = 0.7f;
	...
	for (int iter = 0; iter < 4; ++iter)
	{
		for (int i = 0; i < nagents; ++i)
		{
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				dtFloat dist = dtVlenSqr(diff);
				if (dist > dtSqr(ag->params.radius + nei->params.radius))
					continue;
				dist = dtMathSqrtf(dist);
				dtFloat pen = (ag->params.radius + nei->params.radius) - dist;
				if (dist < 0.0001f)
				{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		// Update position
		const dtFloat ta = anim->tmax*0.15f;
		const dtFloat tb = anim->tmax;
		if (anim->t < ta)
		{
			const dtFloat u = tween(anim->t, 0.0, ta);
			dtVlerp(ag->npos, anim->initPos, anim->startPos, u);
		}
		else
		{
			const dtFloat u = tween(anim->t, ta, tb);
