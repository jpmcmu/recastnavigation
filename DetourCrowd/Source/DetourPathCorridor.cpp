...
One of the difficulties in maintaining a path is that dtFloating point errors, locomotion inaccuracies, and/or local 
...
void dtPathCorridor::reset(dtPolyRef ref, const dtFloat* pos)
{
	dtAssert(m_path);
	dtVcopy(m_pos, pos);
...
*/
int dtPathCorridor::findCorners(dtFloat* cornerVerts, unsigned char* cornerFlags,
							  ...
							  dtNavMeshQuery* navquery, const dtQueryFilter* /*filter*/)
{
	...
	static const dtFloat MIN_TARGET_DIST = 0.01f;
	...
	while (ncorners)
	{
		...
		if (ncorners)
		{
			...
			memmove(cornerVerts, cornerVerts+3, sizeof(dtFloat)*3*ncorners);
		}
	}
...
*/
void dtPathCorridor::optimizePathVisibility(const dtFloat* next, const dtFloat pathOptimizationRange,
										  dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	...
	dtFloat goal[3];
	dtVcopy(goal, next);
	dtFloat dist = dtVdist2D(m_pos, goal);
...
bool dtPathCorridor::moveOverOffmeshConnection(dtPolyRef offMeshConRef, dtPolyRef* refs,
											   dtFloat* startPos, dtFloat* endPos,
...
*/
bool dtPathCorridor::movePosition(const dtFloat* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
*/
bool dtPathCorridor::moveTargetPosition(const dtFloat* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
void dtPathCorridor::setCorridor(const dtFloat* target, const dtPolyRef* path, const int npath)
{
	dtAssert(m_path);
	dtAssert(npath > 0);
...
bool dtPathCorridor::fixPathStart(dtPolyRef safeRef, const dtFloat* safePos)
{
	dtAssert(m_path);
...
bool dtPathCorridor::trimInvalidPath(dtPolyRef safeRef, const dtFloat* safePos,
									 dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	dtAssert(navquery);
	dtAssert(filter);
