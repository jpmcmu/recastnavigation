...
dtLocalBoundary::dtLocalBoundary() :
	...
	m_npolys(0)
{
	dtVset(m_center, DT_FLT_MAX,DT_FLT_MAX,DT_FLT_MAX);
}
...
void dtLocalBoundary::addSegment(const dtFloat dist, const dtFloat* s)
{
	...
	seg->d = dist;
	memcpy(seg->s, s, sizeof(dtFloat)*6);
	
	if (m_nsegs < MAX_LOCAL_SEGS)
		m_nsegs++;
}

void dtLocalBoundary::update(dtPolyRef ref, const dtFloat* pos, const dtFloat collisionQueryRange,
							 dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	m_nsegs = 0;
	dtFloat segs[MAX_SEGS_PER_POLY*6];
	int nsegs = 0;
	for (int j = 0; j < m_npolys; ++j)
	{
		...
		{
			const dtFloat* s = &segs[k*6];
			// Skip too distant segments.
			dtFloat tseg;
			const dtFloat distSqr = dtDistancePtSegSqr2D(pos, s, s+3, tseg);
