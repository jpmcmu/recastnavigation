...
class dtLocalBoundary
{
	...
	struct Segment
	{
		dtFloat s[6];	///< Segment start/end
		dtFloat d;	///< Distance for pruning.
	};
	...
	void addSegment(const dtFloat dist, const dtFloat* s);
...
public:
	...
	void update(dtPolyRef ref, const dtFloat* pos, const dtFloat collisionQueryRange,
				dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	
	bool isValid(dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	
	inline const dtFloat* getCenter() const { return m_center; }
	inline int getSegmentCount() const { return m_nsegs; }
	inline const dtFloat* getSegment(int i) const { return m_segs[i].s; }
