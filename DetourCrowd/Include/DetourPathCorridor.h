...
public:
	...
	void reset(dtPolyRef ref, const dtFloat* pos);
	...
	int findCorners(dtFloat* cornerVerts, unsigned char* cornerFlags,
	...
	///  @param[in]		navquery				The query object used to build the corridor.
	///  @param[in]		filter					The filter to apply to the operation.			
	void optimizePathVisibility(const dtFloat* next, const dtFloat pathOptimizationRange,
								dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	bool fixPathStart(dtPolyRef safeRef, const dtFloat* safePos);

	bool trimInvalidPath(dtPolyRef safeRef, const dtFloat* safePos,
						 dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	bool movePosition(const dtFloat* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	...
	///  @param[in]		filter		The filter to apply to the operation.
	/// @return Returns true if move succeeded.
	bool moveTargetPosition(const dtFloat* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter);
	
	/// Loads a new path and target into the corridor.
	///  @param[in]		target		The target location within the last polygon of the path. [(x, y, z)]
	///  @param[in]		path		The path corridor. [(polyRef) * @p npolys]
	///  @param[in]		npath		The number of polygons in the path.
	void setCorridor(const dtFloat* target, const dtPolyRef* polys, const int npath);
	
	/// Gets the current position within the corridor. (In the first polygon.)
	/// @return The current position within the corridor.
	inline const dtFloat* getPos() const { return m_pos; }

	/// Gets the current target within the corridor. (In the last polygon.)
	/// @return The current target within the corridor.
	inline const dtFloat* getTarget() const { return m_target; }
