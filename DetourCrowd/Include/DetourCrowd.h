...
struct dtCrowdNeighbour
{
	...
	dtFloat dist;		///< The distance between the current agent and the neighbor.
};
...
{
	dtFloat radius;						///< Agent radius. [Limit: >= 0]
	dtFloat height;						///< Agent height. [Limit: > 0]
	dtFloat maxAcceleration;				///< Maximum allowed acceleration. [Limit: >= 0]
	dtFloat maxSpeed;						///< Maximum allowed speed. [Limit: >= 0]

	/// Defines how close a collision element must be before it is considered for steering behaviors. [Limits: > 0]
	dtFloat collisionQueryRange;

	dtFloat pathOptimizationRange;		///< The path visibility optimization range. [Limit: > 0]

	/// How aggresive the agent manager should be at avoiding collisions with this agent. [Limit: >= 0]
	dtFloat separationWeight;
...
struct dtCrowdAgent
{
	...
	/// The desired speed.
	dtFloat desiredSpeed;

	dtFloat npos[3];		///< The current agent position. [(x, y, z)]
	dtFloat disp[3];		///< A temporary value used to accumulate agent displacement during iterative collision resolution. [(x, y, z)]
	dtFloat dvel[3];		///< The desired velocity of the agent. Based on the current path, calculated from scratch each frame. [(x, y, z)]
	dtFloat nvel[3];		///< The desired velocity adjusted by obstacle avoidance, calculated from scratch each frame. [(x, y, z)]
	dtFloat vel[3];		///< The actual velocity of the agent. The change from nvel -> vel is constrained by max acceleration. [(x, y, z)]

	/// The agent's configuration parameters.
	dtCrowdAgentParams params;

	/// The local path corridor corners for the agent. (Staight path.) [(x, y, z) * #ncorners]
	dtFloat cornerVerts[DT_CROWDAGENT_MAX_CORNERS*3];
	...
	dtFloat targetPos[3];					///< Target position of the movement request (or velocity in case of DT_CROWDAGENT_TARGET_VELOCITY).
	dtPathQueueRef targetPathqRef;		///< Path finder ref.
	bool targetReplan;					///< Flag indicating that the current path is being replanned.
	dtFloat targetReplanTime;				/// <Time since the agent's target was replanned.
};

struct dtCrowdAgentAnimation
{
	bool active;
	dtFloat initPos[3], startPos[3], endPos[3];
	dtPolyRef polyRef;
	dtFloat t, tmax;
};
...
class dtCrowd
{
	...
	dtFloat m_agentPlacementHalfExtents[3];

	dtQueryFilter m_filters[DT_CROWD_MAX_QUERY_FILTER_TYPE];

	dtFloat m_maxAgentRadius;

	int m_velocitySampleCount;

	dtNavMeshQuery* m_navquery;

	void updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const dtFloat dt);
	void updateMoveRequest(const dtFloat dt);
	void checkPathValidity(dtCrowdAgent** agents, const int nagents, const dtFloat dt);

	inline int getAgentIndex(const dtCrowdAgent* agent) const  { return (int)(agent - m_agents); }

	bool requestMoveTargetReplan(const int idx, dtPolyRef ref, const dtFloat* pos);
...
public:
	...
	bool init(const int maxAgents, const dtFloat maxAgentRadius, dtNavMesh* nav);
	...
	int addAgent(const dtFloat* pos, const dtCrowdAgentParams* params);
	...
	bool requestMoveTarget(const int idx, dtPolyRef ref, const dtFloat* pos);

	/// Submits a new move request for the specified agent.
	///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	///  @param[in]		vel		The movement velocity. [(x, y, z)]
	/// @return True if the request was successfully submitted.
	bool requestMoveVelocity(const int idx, const dtFloat* vel);
	...
	void update(const dtFloat dt, dtCrowdAgentDebugInfo* debug);
	...
	const dtFloat* getQueryHalfExtents() const { return m_agentPlacementHalfExtents; }

	/// Same as getQueryHalfExtents. Left to maintain backwards compatibility.
	/// @return The search halfExtents used by the crowd. [(x, y, z)]
	const dtFloat* getQueryExtents() const { return m_agentPlacementHalfExtents; }
