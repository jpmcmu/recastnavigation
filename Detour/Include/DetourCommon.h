...
template<class T> inline T dtMax(T a, T b) { return a > b ? a : b; }
 inline dtFloat dtMax(dtFloat a, dtFloat b) { return a > b ? a : b; }
...
inline dtFloat dtClamp(dtFloat v, dtFloat mn, dtFloat mx) { return v < mn ? mn : (v > mx ? mx : v); }
...
inline void dtVcross(dtFloat* dest, const dtFloat* v1, const dtFloat* v2)
{
...
inline dtFloat dtVdot(const dtFloat* v1, const dtFloat* v2)
{
...
inline void dtVmad(dtFloat* dest, const dtFloat* v1, const dtFloat* v2, const dtFloat s)
{
	dest[0] = v1[0]+v2[0]*s;
	dest[1] = v1[1]+v2[1]*s;
	dest[2] = v1[2]+v2[2]*s;
...
inline void dtVlerp(dtFloat* dest, const dtFloat* v1, const dtFloat* v2, const dtFloat t)
{
...
inline void dtVadd(dtFloat* dest, const dtFloat* v1, const dtFloat* v2)
{
	dest[0] = v1[0]+v2[0];
	dest[1] = v1[1]+v2[1];
	dest[2] = v1[2]+v2[2];
...
inline void dtVsub(dtFloat* dest, const dtFloat* v1, const dtFloat* v2)
{
	dest[0] = v1[0]-v2[0];
	dest[1] = v1[1]-v2[1];
	dest[2] = v1[2]-v2[2];
...
inline void dtVscale(dtFloat* dest, const dtFloat* v, const dtFloat t)
{
	dest[0] = v[0]*t;
	dest[1] = v[1]*t;
	dest[2] = v[2]*t;
...
inline void dtVmin(dtFloat* mn, const dtFloat* v)
{
	mn[0] = dtMin(mn[0], v[0]);
	mn[1] = dtMin(mn[1], v[1]);
	mn[2] = dtMin(mn[2], v[2]);
...
///  @param[in]		v	A vector. [(x, y, z)]
inline void dtVmax(dtFloat* mx, const dtFloat* v)
{
	mx[0] = dtMax(mx[0], v[0]);
	mx[1] = dtMax(mx[1], v[1]);
	mx[2] = dtMax(mx[2], v[2]);
...
inline void dtVset(dtFloat* dest, const dtFloat x, const dtFloat y, const dtFloat z)
{
...
inline void dtVcopy(dtFloat* dest, const dtFloat* a)
{
	dest[0] = a[0];
	dest[1] = a[1];
	dest[2] = a[2];
...
inline dtFloat dtVlen(const dtFloat* v)
{
...
inline dtFloat dtVlenSqr(const dtFloat* v)
{
...
inline dtFloat dtVdist(const dtFloat* v1, const dtFloat* v2)
{
	const dtFloat dx = v2[0] - v1[0];
	const dtFloat dy = v2[1] - v1[1];
	const dtFloat dz = v2[2] - v1[2];
...
inline dtFloat dtVdistSqr(const dtFloat* v1, const dtFloat* v2)
{
	const dtFloat dx = v2[0] - v1[0];
	const dtFloat dy = v2[1] - v1[1];
	const dtFloat dz = v2[2] - v1[2];
...
inline dtFloat dtVdist2D(const dtFloat* v1, const dtFloat* v2)
{
	const dtFloat dx = v2[0] - v1[0];
	const dtFloat dz = v2[2] - v1[2];
...
inline dtFloat dtVdist2DSqr(const dtFloat* v1, const dtFloat* v2)
{
	const dtFloat dx = v2[0] - v1[0];
	const dtFloat dz = v2[2] - v1[2];
	return dx*dx + dz*dz;
...
inline void dtVnormalize(dtFloat* v)
{
	dtFloat d = 1.0f / dtMathSqrtf(dtSqr(v[0]) + dtSqr(v[1]) + dtSqr(v[2]));
	v[0] *= d;
	v[1] *= d;
	v[2] *= d;
}
...
inline bool dtVequal(const dtFloat* p0, const dtFloat* p1)
{
	// static const dtFloat thr = dtSqr(1.0f/16384.0f);
	const dtFloat d = dtVdistSqr(p0, p1);
	return d < DT_FLT_EPSILON;
}
...
///
/// The vectors are projected onto the xz-plane, so the y-values are ignored.
inline dtFloat dtVdot2D(const dtFloat* u, const dtFloat* v)
{
...
///
/// The vectors are projected onto the xz-plane, so the y-values are ignored.
inline dtFloat dtVperp2D(const dtFloat* u, const dtFloat* v)
{
...
inline dtFloat dtTriArea2D(const dtFloat* a, const dtFloat* b, const dtFloat* c)
{
	const dtFloat abx = b[0] - a[0];
	const dtFloat abz = b[2] - a[2];
	const dtFloat acx = c[0] - a[0];
	const dtFloat acz = c[2] - a[2];
...
inline bool dtOverlapBounds(const dtFloat* amin, const dtFloat* amax,
...
inline void dtSwapEndian(dtFloat* v)
{
	unsigned char* x = (unsigned char*)v;
	dtSwapByte(x+0, x+3); dtSwapByte(x+1, x+2);
}
...
@fn dtFloat dtTriArea2D(const dtFloat* a, const dtFloat* b, const dtFloat* c)
@par
